...
class CrossEncoderReranker:
    ...
    def compare_with_baseline(
        ...
        score_improvement = (
            ((avg_ce - avg_baseline) / avg_baseline) * 100 if avg_baseline != 0 else 0
        )
...
if __name__ == "__main__":
    ...
    for i, (doc, score) in enumerate(zip(reranked_docs, scores or [], strict=False), 1):
    ...
    print(
        f"Overlap with baseline top-3: {comparison['overlap']}/3 ({comparison['overlap_percentage']:.1f}%)"
    )
