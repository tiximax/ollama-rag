...
class CacheWarmer:
    ...
    def __init__(
        self,
        popular_queries_file: Optional[str] = None,
        analytics_log_dir: Optional[str] = None,
        max_queries: int = 50,
    ):
        """
        Args:
    ...
    def load_popular_queries(self) -> List[str]:
        ...
        if self.popular_queries_file and os.path.exists(self.popular_queries_file):
            try:
                with open(self.popular_queries_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
    ...
    async def warm_cache_async(
        ...
        for query in queries:
            tasks.append(self._warm_single_query_async(
                query, cache, embedder, query_fn, embeddings_only
            ))
