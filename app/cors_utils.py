...
def validate_cors_origins(origins_str: str) -> List[str]:
    ...
    for origin in origins:
        ...
        if "*" in origin:
            raise ValueError(
                f"Wildcard patterns not allowed in CORS origin: {origin}. "
                "Use exact URLs only."
            )
        ...
        if not parsed.scheme:
            raise ValueError(
                f"Origin must include scheme (http:// or https://): {origin}"
            )
        
        if not parsed.netloc:
            raise ValueError(
                f"Origin must include domain/host: {origin}"
            )
        ...
        if clean_origin != origin:
            logger.warning(
                f"CORS origin normalized: '{origin}' → '{clean_origin}'"
            )
