...
def test_bug3_cors_validation():
    ...
    try:
        ...
        if "*" in result:
            ...
            results.add_pass("BUG #3.1 - Wildcard rejection", f"Correctly rejected '*', fallback to {result}")
        
        # Test 2: Valid origins should pass
        result = parse_cors_origins_safe("http://localhost:3000,https://app.com", "http://localhost:8000")
        ...
        result = parse_cors_origins_safe("http://localhost:3000,http://valid.com", "http://localhost:8000")
        if "http://localhost:3000" in result and "http://valid.com" in result:
            results.add_pass("BUG #3.3 - Multiple valid origins", "Multiple valid origins parsed correctly")
        else:
...
def test_bug4_race_condition():
    ...
    try:
        ...
        if engine.build_count == 1:
            results.add_pass("BUG #4 - Race condition", f"BM25 built only once ({engine.build_count}) despite 10 threads")
        else:
            results.add_fail("BUG #4 - Race condition", f"BM25 built {engine.build_count} times (should be 1)")
    ...
    try:
        ...
        for original, should_redact in test_cases:
            ...
            if has_redacted == should_redact:
                passed += 1
                print(f"   ✅ '{original[:30]}...' -> {'REDACTED' if has_redacted else 'UNCHANGED'}")
            else:
                print(f"   ❌ '{original[:30]}...' -> Expected {'REDACTED' if should_redact else 'UNCHANGED'}, got '{record.msg[:40]}...'")
        
        if passed == len(test_cases):
            results.add_pass("BUG #1 - Logging filter", f"All {len(test_cases)} test cases passed")
        else:
            results.add_fail("BUG #1 - Logging filter", f"Only {passed}/{len(test_cases)} cases passed")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if len(chats_data) == 5 and elapsed < 0.1:
                results.add_pass("BUG #8 - Bulk fetch", f"Fetched {len(chats_data)} chats in {elapsed*1000:.1f}ms")
            else:
                results.add_fail("BUG #8 - Bulk fetch", f"Bulk fetch slow or incomplete: {len(chats_data)}/5 in {elapsed*1000:.1f}ms")
    ...
    try:
        ...
        if platform.system() == "Windows":
            if normalized == test_name.lower():
                ...
                results.add_fail("BUG #10 - Windows normalize", f"Expected lowercase, got '{normalized}'")
        else:
            if normalized == test_name:
                results.add_pass("BUG #10 - Unix preserve", f"Case preserved on Unix: '{normalized}'")
            else:
                results.add_fail("BUG #10 - Unix preserve", f"Case changed unexpectedly: '{normalized}'")
...
def test_bug11_null_metadata():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if passed == len(test_cases):
                results.add_pass("BUG #11 - Null filters", f"All {len(test_cases)} test cases passed")
            else:
                results.add_fail("BUG #11 - Null filters", f"Only {passed}/{len(test_cases)} cases passed")
...
def test_bug12_division_by_zero():
    ...
    try:
        ...
        if passed == len(test_cases):
            ...
            results.add_fail("BUG #12 - Safe division", f"Only {passed}/{len(test_cases)} cases passed")
        
        # Test _to_similarity with edge cases
        test_sims = RagEngine._to_similarity([0.0, 1.0, float('inf'), float('nan')])
        if len(test_sims) == 4 and all(0.0 <= s <= 1.0 for s in test_sims if not (s != s)):  # Check valid range
            results.add_pass("BUG #12 - Similarity conversion", "Edge cases handled in similarity conversion")
        else:
            results.add_fail("BUG #12 - Similarity conversion", f"Invalid similarity values: {test_sims}")
